    ...
    return (
        <Box>
            ...
            <Box>
                ...
                <Paragraph weight={FONT_WEIGHT.LIGHT}>
                    This component is used to render text elements to the
                    screen.
                </Paragraph>
            </Box>
            <Flex direction="column">
                <Flex direction="column">
                    ...
                    <Box
                        borderColor={COLORS.GREY.T200.hex}
                        borderStyle="solid"
                        borderWidth={1}
                    ...
                    >
                        {SIZES.map((size, index) => (
                            <Box mb={5} pb={5} key={`paragraph_1_${index}`}>
                                <Paragraph
                                    size={FONT_SIZES[size]}
                                >{`This is ${size.toLowerCase()}`}</Paragraph>
                            </Box>
                        ))}
                    </Box>
                </Flex>
                <Flex direction="column">
                    ...
                    >
                        {WEIGHTS.map((weight, index) => (
                            <Box mb={5} pb={5} key={`paragraph_2_${index}`}>
                                <Paragraph
                                    weight={FONT_WEIGHT[weight]}
                                >{`This is ${weight.toLowerCase()}`}</Paragraph>
                            </Box>
                        ))}
                    </Box>
                </Flex>
