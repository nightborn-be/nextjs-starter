...
const Button = ({ variant = "primary", text, onClick, type = "submit", ...rest }: ButtonProps) => {
	// Functions
	const getBackgroundColor = (state: "normal" | "hovered" | "focused" = "normal") => {
		switch (variant) {
			case "primary":
				switch (state) {
					case "hovered":
	...
	return (
		<ChakraButton border={getBorder()} color={getTextColor()} _hover={{ bg: getBackgroundColor("hovered") }} _focus={{ border: getOutline() }} _active={{ bg: getBackgroundColor("focused") }} bg={getBackgroundColor()} onClick={onClick}>
			{text}
		</ChakraButton>
	);
};
